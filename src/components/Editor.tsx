...
const EditorCard: React.FC<{
  ...
  const [{ isDragging }, drag] = useDrag({
    type: 'editor-card',
    item: () => {
      return { id: card.id, index, isNewCard: card.isNewCard }
    },
    collect: (monitor) => ({
  ...
  const [, drop] = useDrop(() => ({
    ...
    drop(item: IndexCardData & { index?: number, isNewCard?: boolean }, monitor) {
      ...
      if (item.index === undefined) {
        ...
        moveCard(item.index, insertIndex)
      }
    },
  }), [cards, onDrop, moveCard])
